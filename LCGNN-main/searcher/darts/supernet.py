...
class Network_Darts(nn.Module):
    ...
    def _initialize_alphas(self):
        ...
        self.alphas = Variable(1e-3 * torch.randn(self.cluster_num, num_ops).cuda(), requires_grad=True)
        self._arch_parameters = [self.alphas] #这一行将 alphas 添加到架构参数列表 _arch_parameters 中。
    ...
    def _loss(self, x, y, minibatch_info=None, is_valid=True):
        if self.args.use_dmon:
            h_attribute, node_embedding, _, logits, dmon_loss, _ = self(x, minibatch_info)#h_attribute: 节点属性的隐藏表示  node_embedding: 节点的嵌入表示。 logits: 模型的输出，即对每个类别的预测分数  dmon_loss: 模型的损失值。  _: 一个占位符，表示忽略该值。
        else:
    ...
    def forward(self, features_list, mini_batch_input=None, use_dmon=False):
        ...
        if self.args.useTypeLinear:
            ...
            # print(f"gnn_model: {self.gnn_model}")
            # print(f"h_attributed: {h_attributed.shape}")
            # print(f"mini_batch_input: {mini_batch_input}")
            #!!!!!!!!!!!!!!!gnn_model的前向计算！！！！！！！！！！！！！！
