...
class Network_discrete(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        if 'one-hot' in PRIMITIVES:
            # construct one-hot embedding weight matrix  # 构建 one-hot 编码的嵌入权重矩阵
            self.one_hot_feature_list = []  # 存储 one-hot 特征矩阵的列表
            self.embedding_list = nn.ModuleList()  # 存储线性层的列表
            # 遍历所有节点类型
            for i in range(self.all_nodes_type_num):
                dim = self.node_type_split_list[i] # 获取当前节点类型的维度
                # 如果是有效属性节点类型（valid_attr_node_type），则跳过
                if i == self.valid_attr_node_type:
                    self.one_hot_feature_list.append(None)
                    self.embedding_list.append(None)
                    continue
                # 构建 one-hot 特征矩阵的稀疏表示
    ...
    def _loss(self, x, y, is_valid=True):
        # 调用模型的前向传播方法获取节点嵌入和logits
        node_embedding, logits = self(x)
        # 根据 is_valid 决定使用验证集还是训练集的索引
        if is_valid:
