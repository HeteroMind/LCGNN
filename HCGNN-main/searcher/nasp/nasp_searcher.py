...
class NASPSearcher:
    ...
    # def __init__(self, args):
        ...
        # self._writer = SummaryWriter(f'/root/tf-logs/{self._save_dir_name}') #原本的
        #自己改的
    ...
    def _train_search(self):
        ...
        if self.args.use_minibatch is False:
            ...
            if self.args.use_dmon:# 如果使用dmon
                h_attribute, node_embedding, _, logits, dmon_loss, assignments = self._supernet(self.all_input, use_dmon=True)# 获取超网络的输出，包括节点属性、节点嵌入、logits、dmon损失和分配情况
            else:
                h_attribute, node_embedding, _, logits = self._supernet(self.all_input)# 获取超网络的输出，包括节点属性、节点嵌入和logits
            # self._logger.info(f"search logits: {logits}")
            logits_train = logits[self.train_idx].to(device)# 获取训练集上的logits，并移动到GPU上
            # train_loss = self._criterion(logits_train, self.search_target[self.train_idx])
            if self.args.use_dmon:
                train_loss_classification = self._criterion(logits_train, self.train_target) # 计算分类损失
                train_loss = train_loss_classification + self.args.dmon_loss_alpha * dmon_loss# 计算总损失（分类损失加上dmon损失）
            else:
                train_loss = self._criterion(logits_train, self.train_target)
            train_loss.backward() # 反向传播，计算梯度
            self._supernet.restore()# 恢复超网络
            
            nn.utils.clip_grad_norm_(self._supernet.parameters(), self.args.grad_clip)# 对梯度进行裁剪
            self._supernet_optimizer.step()# 更新超网络参数
            ...
            return train_loss_avg, _node_embedding, lr# 返回训练损失平均值、节点嵌入、学习率
    ...
    def search(self):
        ...
        for epoch in range(self.args.search_epoch):
            t_start = time.time()
            # print alphas info
            self._logger.info(f"Epoch: {epoch}\n{self._print_alpha_info}")#这里输出定义的α （alpha）矩阵的信息  就是跑实验代码出现的不断在优化的矩阵
